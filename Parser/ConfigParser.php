...
require_once 'Parser/ConfigParser/Exception.php';
require_once 'Parser/ConfigParser/Outputter/Default.php';
require_once 'Parser/ConfigParser/Renderer/Default.php';
require_once 'Parser/ConfigParser/MessageProvider/Default.php';
 ...
 * $parser = ConfigParser::fromXmlFile('path/to/file.xml');
 *
 * // with an xml definition string
 * $validXmlString = '..your xml string...';
 * $parser = ConfigParser::fromXmlString($validXmlString);
 * </code>
 *
 * @category  Console
 * @package   ConfigParser
...
class ConfigParser
{
     ...
     * @todo move this to ConfigParser_MessageProvider
     */
    public static $errors = array(
        ...
        'action_class_does_not_exists'       => 'action "{$name}" class "{$class}" not found, make sure that your class is available before calling ConfigParser::registerAction()',
     ...
     *
     * @var    object that implements ConfigParser_Renderer interface
     * @access protected
     */
     ...
     *
     * @var ConfigParser_Outputter An outputter
     */
     ...
     *
     * @var ConfigParser_MessageProvider A message provider instance
     */
    ...
    /**
     * An array of ConfigParser_Argument objects.
     *
    ...
    /**
     * An array of ConfigParser_Command objects (sub commands).
     *
     ...
     * convenience.
     *
     * @var ConfigParser The parent instance
     * @todo move ConfigParser::parent to ConfigParser_Command
     */
    public $parent = false;
    ...
    public static $actions = array(
        'IPPort'      => array('ConfigParser_Action_IPPort', true),
        'StoreStringFalse' => array('ConfigParser_Action_StoreStringFalse', true),
        'StoreTrue'   => array('ConfigParser_Action_StoreTrue', true),
        'StoreFalse'  => array('ConfigParser_Action_StoreFalse', true),
        'StoreString' => array('ConfigParser_Action_StoreString', true),
        'StoreInt'    => array('ConfigParser_Action_StoreInt', true),
        'StoreFloat'  => array('ConfigParser_Action_StoreFloat', true),
        'StoreArray'  => array('ConfigParser_Action_StoreArray', true),
        'Callback'    => array('ConfigParser_Action_Callback', true),
        'Counter'     => array('ConfigParser_Action_Counter', true),
        'Help'        => array('ConfigParser_Action_Help', true),
        'Version'     => array('ConfigParser_Action_Version', true),
        'Password'    => array('ConfigParser_Action_Password', true),
        'List'        => array('ConfigParser_Action_List', true),
    );
    ...
    public function __construct(array $params = array()) 
    {
        ...
        */
        ...
        $this->renderer         = new ConfigParser_Renderer_Default($this);
        $this->outputter        = new ConfigParser_Outputter_Default();
        $this->message_provider = new ConfigParser_MessageProvider_Default();
    }
    ...
    /**
     * Method to allow ConfigParser to accept either:
     *  + a custom renderer, 
     *  + a custom outputter,
     ...
     *
     * @return void
     * @throws ConfigParser_Exception if wrong argument passed
     */
    public function accept($instance) 
    {
        if ($instance instanceof ConfigParser_Renderer) {
            if (property_exists($instance, 'parser') && !$instance->parser) {
                $instance->parser = $this;
            }
            $this->renderer = $instance;
        } else if ($instance instanceof ConfigParser_Outputter) {
            $this->outputter = $instance;
        } else if ($instance instanceof ConfigParser_MessageProvider) {
     ...
     *
     * @return ConfigParser The parser instance
     */
    public static function fromXmlFile($file) 
    {
        include_once 'Parser/ConfigParser/XmlParser.php';
        return ConfigParser_XmlParser::parse($file);
    }
     ...
     *   </argument>
     * </command>';
     * $parser = ConfigParser::fromXmlString($xmldata);
     * $result = $parser->parse();
     * </code>
     *
    ...
    {
        include_once 'Parser/ConfigParser/XmlParser.php';
        return ConfigParser_XmlParser::parseString($string);
    }
     ...
     * array $params, then return the ConfigParser_Argument instance
     * created.
     * The method accepts another form: you can directly pass a 
     * ConfigParser_Argument object as the sole argument, this allows
     ...
     *                      instance of ConfigParser_Argument
     * @param array $params An array containing the argument attributes
     *
     * @return ConfigParser_Argument the added argument
     * @see ConfigParser_Argument
     */
    public function addArgument($name, $params = array())
    {
        if ($name instanceof ConfigParser_Argument) {
            $argument = $name;
        } else {
            include_once 'Parser/ConfigParser/Argument.php';
            $argument = new ConfigParser_Argument($name, $params);
        }
        $argument->validate();
     ...
     * ConfigParser_Command instance, you can then populate the command
     * with options, configure it, etc... like you would do for the main parser
     * because the class ConfigParser_Command inherits from
     * ConfigParser.
     *
     * An example:
     * <code>
     ...
     *                      instance of ConfigParser_Command
     * @param array $params An array containing the command attributes
     *
     * @return ConfigParser_Command the added subcommand
     * @see    ConfigParser_Command
     */
    public function addCommand($name, $params = array())
    {
        if ($name instanceof ConfigParser_Command) {
            $command = $name;
        } else {
            include_once 'Parser/ConfigParser/Command.php';
            $params['name'] = $name;
            $command        = new ConfigParser_Command($params);
     ...
     * array $params, then return the ConfigParser_Option instance
     * created.
     * The method accepts another form: you can directly pass a 
     * ConfigParser_Option object as the sole argument, this allows
    ...
    public function addOption($name, $params = array())
    {
        ...
        if ($name instanceof ConfigParser_Option) {
            $opt = $name;
        } else {
            $opt = new ConfigParser_Option($name, $params);
        }
        $opt->validate();
     ...
     *
     * @return mixed A ConfigParser_Option instance or false
     */
     ...
     *
     * class ActionRange extends ConfigParser_Action
     * {
     ...
     * ConfigParser::registerAction('Range', 'ActionRange');
    ...
    private function parseWithConfig($configuration, $result)
    {
        ...
        foreach ($this->options as $name=>$option) 
        {
            if(is_array($option->path))
            {
                ...
                {
                    ($setting = self::accessArrayElementByPath($configuration, $path)) ?: $setting = $option->default[$config];
            ...
            {
                ($setting = self::accessArrayElementByPath($configuration, $option->path)) ?: $setting = $option->default;
                $value = &$setting;
                $this->_dispatchAction($option, $value, $result);
                //$result->options[$name] = $setting;
            }
        }
        ...
        {
            $output .= self::sprintfn($line, $result->options).PHP_EOL;
        }
        return $output;
    }
    // }}}
    ...
    public function parse($userArgc=null, $userArgv=null)
    {
        ...
        */
        ...
        include_once 'Parser/ConfigParser/Result.php';
        $result = new ConfigParser_Result();
        
        /*
        if (!($this instanceof ConfigParser_Command)) {
        ...
        */
        ...
        //$args = array();
        //$configList = array_keys($this->configuration);
        //if(is_numeric($configList[0]))
        ...
        //$result->parsed = self::sprintfn($this->template, $result->options);
     ...
     * @param object $result The ConfigParser_Result instance
        ...
        if (php_sapi_name() != 'cli') {
            ...
            if (isset($_REQUEST)) {
                foreach ($_REQUEST as $key => $value) {
                    ...
                    if ($opt instanceof ConfigParser_Option) {
    ...
    */
     ...
     *
     * @param ConfigParser_Option $option The option instance
     ...
     *
     * @return mixed An instance of ConfigParser_Command or false
     */
