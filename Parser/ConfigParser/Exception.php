 ...
 */
require_once 'Parser/ConfigParser/CustomMessageProvider.php';

/**
 * Class for exceptions raised by the ConfigParser package.
 *
 * @category  Console
 * @package   ConfigParser
 * @author    David JEAN LOUIS <izimobil@gmail.com>
 ...
 */
class ConfigParser_Exception extends PEAR_Exception
{
    // Codes constants {{{
     ...
     * @param ConfigParser $parser   An instance of the parser
     * @param array               $messages An optional array of messages
     *                                      passed to the message provider.
     *
     * @return object an instance of ConfigParser_Exception
     */
    public static function factory(
        $code, $params, $parser, array $messages = array()
    ) {
        $provider = $parser->message_provider;
        if ($provider instanceof ConfigParser_CustomMessageProvider) {
        ...
        $const = 'ConfigParser_Exception::' . $code;
        $code  = defined($const) ? constant($const) : 0;
        return new ConfigParser_Exception($msg, $code);
    }
