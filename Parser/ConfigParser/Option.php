...
require_once 'Parser/ConfigParser/Element.php';
 ...
 */
class ConfigParser_Option extends ConfigParser_Element
{
    // Public properties {{{
    ...
    {
        $actionInfo = ConfigParser::$actions[$this->action];
        if (true === $actionInfo[1]) {
            // we have a "builtin" action
            $tokens = explode('_', $actionInfo[0]);
            include_once 'Parser/'.implode('/', $tokens) . '.php';
        }
    ...
    public function validate()
    {
        ...
        if ($this->short_name == null && $this->path == null) {
...
/*        if ($this->short_name != null && 
            ...
            ConfigParser::triggerError('option_bad_short_name',
                E_USER_ERROR, array(
                    '{$name}' => $this->name, 
        ...
        if ($this->long_name != null && 
            ...
            ConfigParser::triggerError('option_bad_long_name',
                E_USER_ERROR, array(
                    '{$name}' => $this->name, 
        ...
        if (!isset(ConfigParser::$actions[$this->action])) {
     ...
     * default behaviour in the next major release of ConfigParser.
     *
     * @return void
     */
